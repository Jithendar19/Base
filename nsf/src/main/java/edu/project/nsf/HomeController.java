...
public class HomeController {
	...
	// ************************************************************************************************************************//
	// object of Pojo class RnaVariables is initialized and passed as a
	// parameter to the constructor of NSFRNA object creation.//
	// ************************************************************************************************************************//
	...
	// *****************************************************************************//
	// servet returns the home page when the /nsf request is made to the
	// controller.
	// *****************************************************************************//
	...
	// ************************************************************************************************************//
	// once user enters the pattern file after file validation user is
	// redirected to search with list of matches
	// ************************************************************************************************************//
	@RequestMapping(value = "/searchResults", method = RequestMethod.POST)
	public String search(Locale locale, Model model, @RequestParam("name") String name,
			@RequestParam("file") MultipartFile file, @RequestParam("searchType") int searchType,
			HttpServletRequest request) {
		System.out.println("IP Address: " + request.getRemoteAddr() + " has accessed the searchResults page\"");

		var.setNucleotide(searchType);

		/*
		 * This list hold the info like rnaName, nucleotide, starting indices of
		 * the pattern in the RNA with each matching patterns.
		 */
		...
		// If the file is empty then user is returned to upload page with error
		// msg.
		...
		if (!file.isEmpty()) {
			try {
				...
				BufferedOutputStream stream = new BufferedOutputStream(
						new FileOutputStream(new File(searchedPath + name)));
				...
				// var.setSearchFileName(file.toString());
				var.setSearchFileName(searchedPath + name);
				System.out.println(file.toString() + " has been instatiated in the RNAVariables class");
				...
				// Once the search is complete an output string is split and
	...
	public String comparison(Locale locale, Model model, @RequestParam("results") String results,
			@RequestParam("searchType") int searchType, HttpServletRequest request) {
		System.out.println("\nIP Address: " + request.getRemoteAddr() + " has accessed the Comparison Results Page.");
		System.out.println("Files to be compared: " + results);
		var.setCompareWithNucleotide(searchType);
		...
		var.setCompareRNANames(rnacmp.GetCompareRNANames(var.getCompareRNAString(), var.getNumOfCompareRNAs(),
				var.getCompareRNANames()));
		var.setCompareRNANums(rnacmp.GetCompareRNAs(var.getCompareRNANames(), var.getCompareRNANums(),
				var.getNumOfCompareRNAs(), var.getRNANames(), var.getFileNumber()));
		var.setCompareRNAs(rnacmp.ConvertCompareRNAs(var.getCompareRNANums(), var.getCompareRNAs(),
				var.getNumOfCompareRNAs(), var.getCompareWithNucleotide(), var.getConcatenatedFileName(),
				var.getInputRNANucleotide(), var.getInputRNAStructure()));
		long startTime = System.nanoTime(); // Timing the comparison method
		var.setLengthOfCompareRNAs(rnacmp.FindLengthOfComapreRNAs(var.getCompareRNAs(), var.getNumOfCompareRNAs(),
				var.getLengthOfCompareRNAs()));
		var.setSmallestRNAIndex(rnacmp.FindSmallestRNA(var.getSmallestRNAIndex(), var.getLengthOfCompareRNAs(),
				var.getNumOfCompareRNAs()));
		var.setLengthOfSmallestRNA(rnacmp.FindLengthOfSmallestRNA(var.getSmallestRNAIndex(),
				var.getLengthOfSmallestRNA(), var.getLengthOfCompareRNAs()));
		var.setSearchCompareRNA(
				rnacmp.SearchCompareRNA(var.getSearchCompareRNA(), var.getCompareRNAs(), var.getSmallestRNAIndex()));
		var.setOptimalLength(rnacmp.Compare(var.getSearchCompareRNA(), var.getLengthOfSmallestRNA(),
				var.getCompareRNANums(), var.getCompareWithNucleotide(), var.getSuffixNucleotide(),
				var.getSuffixStructure(), var.getInputRNAStructure(), var.getInputRNANucleotide(),
				var.getSuffixArrayIndexIteratorNucleotide(), var.getSuffixArrayIndexIteratorStructure(),
				var.getMatchingRNAs(), var.getNumOfCompareRNAs()));
		long endTime = System.nanoTime();
		long duration = (endTime - startTime);
		System.out.println("Time for Comparison :" + duration);
		var.setOptimalPattern(rnacmp.FindOptimalPattern(var.getOptimalPattern(), var.getOptimalLength(),
				var.getSearchCompareRNA(), var.getSuffixNucleotide(), var.getSuffixStructure(),
				var.getInputRNAStructure(), var.getInputRNANucleotide(), var.getSuffixArrayIndexIteratorNucleotide(),
				var.getSuffixArrayIndexIteratorStructure(), var.getCompareWithNucleotide(), var.getCompareRNANums(),
				var.getNumOfCompareRNAs(), var.getLengthOfSmallestRNA()));
		var.setCompareOutputString(rnacmp.GeneratesetCompareOutputString(var.getOptimalPattern(), var.getSuffixNucleotide(), var.getSuffixStructure(),
				var.getInputRNAStructure(), var.getInputRNANucleotide(), var.getSuffixArrayIndexIteratorNucleotide(),
				var.getSuffixArrayIndexIteratorStructure(), var.getOptimalLength(), var.getCompareWithNucleotide(),
				var.getCompareRNANums(), var.getNumOfCompareRNAs()));
	 ...
	 * model, @RequestParam("name") String name,
	 * 
	 * @RequestParam("file") MultipartFile file, HttpServletRequest request) {
	 * System.out.println("IP Address: " + request.getRemoteAddr() +
	 * " has accessed the Regular Expression Search Results Page."); ReadFile
	 * test = new ReadFile();
	 * 
	 * if (!file.isEmpty()) { try { byte[] bytes = file.getBytes();
	 * BufferedOutputStream stream = new BufferedOutputStream(new
	 * FileOutputStream(new File(name))); stream.write(bytes); stream.close();
	 * 
	 * model.addAttribute("text", dataBase); model.addAttribute("msg", name);
	 * model.addAttribute("RegularExpPojo", new RegularExpPojo());
	 * 
	 * return "RegularExp";
	 * 
	 * } catch (Exception e) { model.addAttribute("msg", "You failed to upload "
	 * + name + " => " + e.getMessage()); model.addAttribute("RegularExpPojo",
	 * new RegularExpPojo()); model.addAttribute("text", test.read(name));
	 * return "upload"; } } else { model.addAttribute("msg",
	 * "You failed to upload " + name + " because the file was empty."); return
	 * "upload"; } }
	 * 
	 * @RequestMapping(value = "/search", method = RequestMethod.POST) public
	 * String regularExpSearch(@ModelAttribute RegularExpPojo pojo, Locale
	 * locale, Model model,
	 * 
	 * @RequestParam("pattern") String pattern, @RequestParam("foundOnFile")
	 * boolean foundOnFile,
	 * 
	 * @RequestParam("path") String filename, HttpServletRequest request) {
	 * System.out.println("IP Address: " + request.getRemoteAddr() +
	 * " has accessed the Suffix Array Search Results Page."); // RegularExp r =
	 * new RegularExp(); String output = new String();
	 * 
	 * System.out.println("Reading database ..... "); StringBuffer sb = new
	 * StringBuffer(dataBase);
	 * 
	 * if (pattern == "") { model.addAttribute("text", sb);
	 * model.addAttribute("msgs", "Empty search not a valid search!");
	 * model.addAttribute("msg", filename); model.addAttribute("RegularExpPojo",
	 * new RegularExpPojo()); return "RegularExp"; }
	 * 
	 ...
	 * System.out.println("pattern searched completed stopping timer."); long
	 * endTime = System.nanoTime(); long duration = (endTime - startTime); long
	 * elapsedTime = (duration);
	 * System.out.println("spliting the search result"); String[] parts =
	 * output.split("#"); int k = Integer.parseInt(parts[parts.length - 3]);
	 * 
	 * System.out.println("the loop begins now: " + k); int count = 0;
	 * 
	 * for (int i = 0; i < (k * 2); i++) {
	 * 
	 * // System.out.println(parts[i]); if (i % 2 == 0) {
	 * sb.insert(Integer.parseInt(parts[i]) + count,
	 * "<font size=\"3\" color=\"red\">"); count = count + 27; } else {
	 * sb.insert(Integer.parseInt(parts[i]) + count, "</font>"); count = count +
	 * 7; } }
	 * 
	 * System.out.println("loop terminated  adding model attributes ....."); if
	 * (foundOnFile) model.addAttribute("status", r.SaveList()); //
	 * System.out.println(sb); model.addAttribute("text", sb);
	 * model.addAttribute("msgs", "Finding matches took: " + elapsedTime +
	 * " nanoseconds"); model.addAttribute("msg", filename);
	 * model.addAttribute("noOfMatches", k); model.addAttribute("pattern",
	 * pattern); model.addAttribute("RegularExpPojo", new RegularExpPojo());
	 * 
	 * System.out.println("Finding matches took: " + elapsedTime +
	 * " nanoseconds");
	 * 
	 * return "RegularExp"; }
	 * 
	 * @RequestMapping(value = "/addRNA", method = RequestMethod.POST) public
	 * String handleRNAUpload(@ModelAttribute RegularExpPojo pojo, Model
	 * model, @RequestParam("name") String name,
	 * 
	 * @RequestParam("file") MultipartFile file, HttpServletRequest request) {
	 * 
	 ...
	 * new RegularExpPojo());
	 * 
	 * return "searchRNA"; } } else { model.addAttribute("msg",
	 * "You failed to upload " + name + " because the file was empty."); return
	 * "uploadfile"; } }
	 * 
	 * @RequestMapping(value = "/RNASearch", method = RequestMethod.POST) public
	 * String SuffixSearch(@ModelAttribute RegularExpPojo pojo, Locale locale,
	 * Model model,
	 * 
	 * @RequestParam("path") String filename, HttpServletRequest request) {
	 * 
